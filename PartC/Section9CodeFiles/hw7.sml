...
(* helper function for 'Shift' expression *)
fun shift (dx,dy,e) = 
	case e of
		NoPoints => NoPoints
	  ...
	  | Shift(dx,dy,e) => shift (dx,dy,e)
	  | _ => raise Impossible("illegal use of 'Shift'")
...
fun eval_prog (e,env) =
    case e of
      ...
      | Shift(dx,dy, e) => eval_prog(shift(dx, dy, (eval_prog (e, env))), env)
...
(* make sure 1) all line segments are not points, and 2) first endpoint is 
"in front of" second endpoint *)
fun preprocess_prog e =
	case e of
		LineSegment(x1,y1,x2,y2) =>
			(*if line segment is basically a point, replace it with a point*)
			if real_close(x1,x2) andalso real_close(y1,y2)
			then Point(x1,y1)
			(*for remaining: if 2nd endpoint is in front of 1st, swap endpoints*)
