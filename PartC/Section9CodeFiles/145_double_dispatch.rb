...
class Int < Value
  ...
  def addRational v # second dispatch: other is MyRationalVal
    MyRationalVal.new(v.i+v.j*i,v.j)
  end
end
...
class MyString < Value
  ...
  def addRational v # second dispatch: other is MyRationalVal (notice order flipped)
    MyString.new(v.i.to_s + "/" + v.j.to_s + s)
  end
end

class MyRationalVal < Value
  attr_reader :i, :j
  def initialize(i,j)
    @i = i
    @j = j
  ...
  def noNegConstants
    if i < 0 && j < 0
      MyRationalVal.new(-i,-j)
    elsif j < 0
      Negate.new(MyRationalVal.new(i,-j))
    elsif i < 0
      Negate.new(MyRationalVal.new(-i,j))
    else
      self
